...
from collector.utils.shared import DEFAULT_DB_URL, DEFAULT_REPOS_FILE, REQUEST_TIMEOUT_SECONDS, parse_next_last
...
def fetch(
    ...
    with progress, httpx.Client(verify=False, timeout=REQUEST_TIMEOUT_SECONDS) as client, output.open("w", encoding="utf-8") as file:
        ...
        while True:
            ...
            response = request_with_retries(client, "GET", f"{registry}{route}", params=params)
            ...
            pages += 1
            progress.update(task_id, pages=pages, total=total, last=(current_last or "-")[:40])
